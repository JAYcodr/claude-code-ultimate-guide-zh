...
CLAUDE.md 文件是持久性指令，Claude 在每次会话开始时都会读取它们。它们被称为"记忆"文件——为 Claude 提供你的偏好、约定和项目上下文的长期记忆，跨会话持久存在，不会每次对话后就被遗忘。
