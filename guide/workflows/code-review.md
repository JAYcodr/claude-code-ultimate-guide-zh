...
Claude Code 对每个 GitHub Pull Request 运行多智能体评审。智能体并行分析 diff 和代码上下文，每个关注不同类型的问题（逻辑错误、安全漏洞、边缘情况等），最后验证步骤过滤误报。

发现的问题在对应代码行上以内联评论形式发布，标注严重性。评审不会自动批准或阻止 PR，保留现有评审流程。
